...
contract InitializeTest is BaseTest {
    function test_failOnLogicContractDirectly() public {
        ...
        _metrom.initialize(address(1), address(1), 10, 10, 10, 11);
    }
    ...
    function test_failZeroAddressOwner() public {
        ...
        _metrom.initialize(address(0), address(0), 10, 10, 10, 10);
    }

    function test_failZeroAddressUpdater() public {
        MetromHarness _metrom = MetromHarness(address(new ERC1967Proxy(address(new MetromHarness()), bytes(""))));
        vm.expectRevert(IMetrom.ZeroAddressUpdater.selector);
        _metrom.initialize(address(1), address(0), 10, 10, 10, 10);
    }

    function test_failInvalidCreationFee() public {
        MetromHarness _metrom = MetromHarness(address(new ERC1967Proxy(address(new MetromHarness()), bytes(""))));
        vm.expectRevert(IMetrom.InvalidFee.selector);
        _metrom.initialize(address(1), address(1), uint32(UNIT), 10, 10, 10);
    }

    function test_failInvalidReimbursementFee() public {
        MetromHarness _metrom = MetromHarness(address(new ERC1967Proxy(address(new MetromHarness()), bytes(""))));
        vm.expectRevert(IMetrom.InvalidFee.selector);
        _metrom.initialize(address(1), address(1), 10, uint32(UNIT), 10, 10);
    }
    ...
    function test_failInvalidMinimumCampaignDuration() public {
        ...
        _metrom.initialize(address(1), address(1), uint32(10_000), 10, 10, 8);

        _metrom = MetromHarness(address(new ERC1967Proxy(address(new MetromHarness()), bytes(""))));
        // minimum campaign duration == than maximum campaign duration
        vm.expectRevert(IMetrom.InvalidMinimumCampaignDuration.selector);
        _metrom.initialize(address(1), address(1), uint32(10_000), 10, 10, 10);
    }

    function test_success() public {
        address _owner = address(1);
        address _updater = address(2);
        uint32 _creationFee = 10;
        uint32 _reimbursementFee = 20;
        ...
        vm.assertEq(_metrom.creationFee(), _creationFee);
        vm.assertEq(_metrom.reimbursementFee(), _reimbursementFee);
    ...
    function testFuzz_success(
        ...
        vm.assume(_creationFee < UNIT);
        vm.assume(_reimbursementFee < UNIT);
