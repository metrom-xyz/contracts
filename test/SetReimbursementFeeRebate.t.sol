...
contract SetReimbursementFeeRebate is BaseTest {
    function test_failZeroAddressAccount() public {
        vm.expectRevert(IMetrom.ZeroAddressAccount.selector);
        metrom.setReimbursementFeeRebate(address(0), uint32(UNIT - 1));
    }

    function test_failRebateTooHigh() public {
        vm.expectRevert(IMetrom.RebateTooHigh.selector);
        metrom.setReimbursementFeeRebate(address(1), uint32(UNIT + 1));
    }

    function test_failForbidden() public {
        vm.expectRevert(IMetrom.Forbidden.selector);
        metrom.setReimbursementFeeRebate(address(1), 10);
    }

    function test_success() public {
        address _account = address(1);

        vm.assertEq(metrom.reimbursementFeeRebate(_account), 0);

        uint32 _newFeeRebate = uint32(10_000);
        vm.prank(owner);
        metrom.setReimbursementFeeRebate(_account, _newFeeRebate);

        vm.assertEq(metrom.reimbursementFeeRebate(_account), _newFeeRebate);
    }
    ...
    function test_successNone() public {
        ...
        vm.prank(owner);
        metrom.setReimbursementFeeRebate(_account, uint32(0));
